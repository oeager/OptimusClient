package com.bsince.optimus.data.entity;
...
public class JsonStreamerEntity implements HttpEntity {
    ...
    private final ResponseHandler progressHandler;
    
    private final Event<?> mEvent;

    public JsonStreamerEntity(Event<?> event,ResponseHandler progressHandler,  boolean useGZipCompression, String elapsedField) {
    ...
    public void writeTo(final OutputStream out) throws IOException {
            ...
            for (String key : keys) {
                ...
                try {
                    ...
                    if (value == null) {
                        ...
                        isFileWrapper = value instanceof MulitDataSet.FileWrapper;

                        // If a file should be uploaded.
                        if (isFileWrapper || value instanceof MulitDataSet.StreamWrapper) {
                            // All uploads are sent as an object containing the file's details.
                            os.write('{');

                            // Determine how to handle this entry.
                            if (isFileWrapper) {
                                writeToFromFile(os, (MulitDataSet.FileWrapper) value);
                            } else {
                                writeToFromStream(os, (MulitDataSet.StreamWrapper) value);
                            }
        ...
        os.flush();
        OptimusClient.silentCloseOutputStream(os);
    }

    private void writeToFromStream(OutputStream os, MulitDataSet.StreamWrapper entry)
            throws IOException {
        ...
        OptimusClient.silentCloseOutputStream(bos);
        ...
        if (entry.autoClose) {
        	...
        	OptimusClient.silentCloseInputStream(entry.inputStream);
        }
    }

    private void writeToFromFile(OutputStream os, MulitDataSet.FileWrapper wrapper)
            throws IOException {
        ...
        while ((bytesRead = in.read(buffer)) != -1) {
            ...
            if(mEvent.loadingProcessor!=null){
            	progressHandler.responProcessing(mEvent, bytesWritten, totalSize);
            }
        }
        ...
        // Safely close the input stream.
        OptimusClient.silentCloseInputStream(in);
    }
