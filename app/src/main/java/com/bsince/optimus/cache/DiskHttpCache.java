...
public class DiskHttpCache implements HttpCache {
	...
	public DiskHttpCache(File rootDirectory, int maxCacheSizeInBytes) {
		mRootDirectory = rootDirectory;
		mMaxCacheSizeInBytes = maxCacheSizeInBytes;
	}

	
	public DiskHttpCache(File rootDirectory) {
	...
	public synchronized Entry get(String key) {
		...
		try {
			...
			L.d(String.format("%s: %s", file.getAbsolutePath(), e.toString()));
			remove(key);
			return null;
		} finally {
			if (cis != null) {
	...
	public synchronized void init() {
		if (!mRootDirectory.exists()) {
			if (!mRootDirectory.mkdirs()) {
				L.e("Unable to create cache dir "+mRootDirectory.getAbsolutePath());
			}
			return;
		}
	...
	public synchronized void put(String key, Entry entry) {
		...
		if (!deleted) {
			L.d("Could not clean up file "+ file.getAbsolutePath());
		}
	}
	...
	public synchronized void remove(String key) {
		...
		removeEntry(key);
		if (!deleted) {
			L.d(String.format("Could not delete cache entry for key=%s, filename=%s",
	...
	private void pruneIfNeeded(int neededSpace) {
		...
		L.v("pruned "+prunedFiles+" files "+(mTotalSize - before)+" bytes, "+(SystemClock.elapsedRealtime()- startTime)+" ms");
	}
	...
	static class CacheHeader {
		...
		public CacheHeader(String key, Entry entry) {
			...
			this.lastModified = entry.lastModifiedTime;
		...
		public Entry toCacheEntry(byte[] data) {
			...
			e.data = data;
			e.etag = etag;
			e.lastModifiedTime = lastModified;
	...
	private static int read(InputStream is) throws IOException {
		...
		if (b == -1) {
			L.e("read to last.......");
			throw new EOFException("diskBaseCache ---error");
		}
		return b;
	}
